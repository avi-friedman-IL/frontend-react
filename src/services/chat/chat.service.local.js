...
async function query(filterBy = { txt: '' }) {
  var chats = await storageService.query(STORAGE_KEY)

  if (!chats || !chats.length) {
    chats = await _createChats()
    storageService.saveToStorage(STORAGE_KEY, chats)
  }
  return chats
}

async function getById(chatId) {
  const chat = await storageService.get(STORAGE_KEY, chatId)
